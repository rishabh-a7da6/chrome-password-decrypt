...
def get_chrome_secret_key():
    """
    Retrieves the secret key used by Chrome browser.

    Returns:
        bytes: The secret key used by the browser, or None if it cannot be found.
    """
    try:
        # Read Chrome local state file
        ...
        print("Error: %s" % str(e))
        print("Error: Chrome secret key could not be found.")
...
def decrypt_password(ciphertext, secret_key):
    ...
    try:
        ...
        print("Error: %s" % str(e))
        print("Error: Unable to decrypt the password. Chrome version <80 is not supported. Please check.")
        return ""

    
def get_db_connection(chrome_login_db_path):
    """
    Copies the chrome login database file to a local location and establishes a connection to it.

    Args:
        chrome (str): The path to the chrome login database file.

    Returns:
        sqlite3.Connection: The connection object to the copied database file, or None if the database cannot be found.
    """
    try:
        shutil.copy2(chrome_login_db_path, "passwords.db")

        return sqlite3.connect("passwords.db")
    
    except Exception as e:
        print("Error: %s" % str(e))
        print("Error: chrome database cannot be found.")
